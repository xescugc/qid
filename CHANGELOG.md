...
- Add `secret_type` and `secret` blocks for injecting secrets into pipeline steps. Secret types define how to fetch secrets via a `get` command, and secrets declare which secret to fetch with specific params. Steps declare `secrets = ["type.name"]` to have secret values injected as `secret_<key>` environment variables before execution. Includes built-in `pikoci://vault` and `pikoci://file` secret types. Supports `source` for remote secret type definitions ([#12](https://github.com/xescugc/pikoci/issues/12))
- Drop CockroachDB and TiDB support. Supported databases are now mem, sqlite, mysql, and postgresql
- Fix PostgreSQL support: duplicate constraint names in migrations, `LastInsertId` not supported by driver, string quoting in seed data
- Fix MySQL/MariaDB migration V8: foreign key constraint error when renaming pipelines table
- Refactor test infrastructure: `make test` runs mem/sqlite, `make test-all` runs all backends (requires `make test-services-up`)
