...
func Handler(s pikoci.Service, ts []byte, l *slog.Logger) http.Handler {
	...
	api.Methods(http.MethodGet).Path("/teams/{team_canonical}/pipelines/{pipeline_name}/jobs/{job_name}/builds/{build_id}").Name(GetJobBuild.String()).Handler(getJobBuild(s))
	api.Methods(http.MethodPost).Path("/teams/{team_canonical}/pipelines/{pipeline_name}/jobs/{job_name}/builds/{build_id}/cancel").Name(CancelJobBuild.String()).Handler(cancelJobBuild(s))
