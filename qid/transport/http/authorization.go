...
type authorizationFn func(ctx context.Context, s qid.Service, un, tc string) error
...
func nothing(ctx context.Context, s qid.Service, un, tc string) error { return nil }

func admin(ctx context.Context, s qid.Service, un, tc string) error {
...
func member(ctx context.Context, s qid.Service, un, tc string) error {
	um, err := s.GetUser(ctx, un)
	if err != nil {
		return fmt.Errorf("failed to GetUser: %w", err)
	}
